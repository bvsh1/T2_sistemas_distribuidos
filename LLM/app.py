...
logger = logging.getLogger("FLINK_SCORING_JOB")
...
logger.info("FLINK: Iniciando Job de Flink (Scoring)...")
logger.info(f"  FLINK_CFG: Consumiendo de: {TOPIC_INPUT}")
logger.info(f"  FLINK_CFG: Publicando (OK) en: {TOPIC_OUTPUT_VALIDATED}")
logger.info(f"  FLINK_CFG: Publicando (Regen) en: {TOPIC_OUTPUT_REGENERATE}")
logger.info(f"  FLINK_CFG: Umbral de Score: {SCORE_THRESHOLD}")
logger.info(f"  FLINK_CFG: Máx. Regeneraciones: {MAX_REGENERATIONS}")
...
def calculate_score(message: dict) -> float:
    ...
    for phrase in BAD_PHRASES_EN: 
        if phrase in clean_a_lower:
            ...
            logger.warning(f"SCORER: Respuesta (ID: {message.get('id', 'N/A')}) penalizada por frase: '{phrase}'")
            break
...
class Scorer(object):
    ...
    def __call__(self, json_string: str) -> dict:
        try:
            ...
            logger.info(f"SCORER: Mensaje puntuado (ID: {message.get('id', 'N/A')}, Score: {score:.2f})")
            return message
        except Exception as e:
            logger.error(f"SCORER: Error fatal parseando/puntuando JSON: {e} - String: {json_string[:100]}")
...
class PrepareForRegeneration(object):
    ...
    def __call__(self, message: dict) -> str:
        ...
        logger.warning(f"REGENERADOR: Enviando a regenerar (ID: {message.get('id')}, Score: {message.get('score'):.2f}, Próximo Intento: {regens})")
